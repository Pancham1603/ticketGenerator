...
Args in __new__: ('***REMOVED***',)
type(self) in __init__: <class 'C'>
Args in __init__: ('***REMOVED***',)

When we call ``C('***REMOVED***')``, the ``__new__`` method gets its own class
as first argument, and the passed argument, which is the string
``'***REMOVED***'``.  After python calls ``__new__``, it usually (see below)
